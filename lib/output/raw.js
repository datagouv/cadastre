...
class FilesStorage {
  ...
  handleFeature({layer, properties, extraProperties, geometry}) {
    if (this.layers && !this.layers.includes(layer)) return
    if (!['COMMUNE', 'SECTION'].includes(layer) || this.registerUnique(properties.IDU)) {
      this.writeTo({type: 'Feature', geometry, properties, extraProperties}, layer.toLowerCase())
    }
  }
...
async function createWriter(destPath, layers) {
  destPath = join(destPath, 'raw')
  await ensureDirectoryExists(destPath)
  return new FilesStorage(destPath, layers)
}

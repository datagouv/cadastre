...
const layersHandlers = {
  ...
  SUBDFISC: {renamedInto: 'subdivisions_fiscales', model: models.prepareSubdivisionFiscale, unique: true}
}
...
class FilesStorage {
  ...
  getStream(fileName) {
    ...
    if (!(fileName in this.streams)) {
      this.streams[fileName] = createGeoJSONWriteStream(filePath).on('error', boom)
    }
    ...
    try {
      ...
      if (handler.unique) {
        if (this.registerUnique(obj.id)) {
          ...
          console.error(`Duplicates ${targetLayerName}: ${obj.id}`)
        }
      } else {
        this.writeTo(obj, targetLayerName)
      }

...
function parseLotFileName(fileName, depCode) {
  const codeCommune = (depCode.startsWith('97') ? '97' : depCode) + fileName.substr(8, 3)
      ...
      return decompress(path, tmpPath, { strip: 5 })
        .then(files => {
          ...
          return Promise.mapSeries(files, file => {
            const { codeCommune, planche } = parseLotFileName(file.path, depCode)

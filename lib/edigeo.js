...
const debug = require('debug')('import-cadastre')
...
function extractHeader(line) {
  return {
    ...
    valueSize: parseInt(line.substr(5, 2)),
  }
}
...
const FORMAT_PARSERS = {
  ...
  I: rawValue => parseInt(rawValue),
  N: rawValue => parseInt(rawValue),
  D: rawValue => {
    if (rawValue.length !== 8) {
      // console.log('[WARN] date not parseable: %s', rawValue)
      return rawValue
    }
...
function parseValue({ valueType, valueFormat, valueSize }, rawValue) {
  ...
  if (valueType === 'C') return rawValue.split(';').filter(v => !!v).map(formatParse)
...
class DescriptorBlock {
  ...
  addToRegistry() {
    ...
    } else if (this.id in this.registry) {

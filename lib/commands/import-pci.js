...
async function handler(archivesPath, targetPath, bundleTypeName) {
  ...
  await Promise.all(depArchives.map(async depArchive => {
    ...
    await Promise.each(files, async file => {
      ...
      // Image
      if (!bundleType.image && bundleType.isImageFeuille(file.path)) {

...
const {booleanPointInPolygon, centroid} = require('@turf/turf')
...
async function handler({rtsPath, parcellesPath, sectionsPath}, distPath) {
  ...
  // Recreate all communes directories (override existing)
  await Promise.all(
    codesCommunes.map(codeCommune => recreateDirectory(communePath(distPath, codeCommune)))
  )
  ...
  await Promise.all(layersToExtract.map(async layerName => {
    const layerDefinition = LAYERS_MAPPING[layerName]
    ...
    const byCommune = groupBy(features, f => {
      ...
      console.log(`Objet de la couche ${layerName} sans commune`)
    })

    await writeLayer(distPath, byCommune, layerDefinition.layer, layerDefinition)
...
function writeLayer(distPath, groupedFeatures, layerName, {convertFn, validateFn}) {
  return Promise.all(
    Object.keys(groupedFeatures)
      .filter(communeIsNotIgnored)
      .map(codeCommune => {
        const geojsonStream = createPolygonWriteStream(getLayerPath(distPath, codeCommune, layerName))
        return writeFeatures(groupedFeatures[codeCommune], geojsonStream, f => convertFn({...f, codeCommune}), validateFn)
      })
  )
}

function writeFeatures(features, geojsonStream, convertFn, validateFn) {
  return new Promise((resolve, reject) => {
    ...
    features.forEach(f => {
      const feature = convertFn(f)
      if (!validateFn || validateFn(feature)) {
        geojsonStream.write(feature)
      } else {
        console.log(`objet invalide : ignoré`)
      }
    })
    geojsonStream.end()
  })
